...
export async function runAgent(promptId: string, prompt: string, deps: AgentDeps): Promise<void> {
  ...
  if (!config.deepseekApiKey) {
    send({
      type: "error",
      id: promptId,
      message: "Kryct Agent isn't configured on the daemon.",
    });
  ...
  try {
    for (let step = 0; step < MAX_STEPS; step++) {
      ...
      let assistantText = "";
      let stepTokens = 0;
      // Prompt/completion split — API-billed runs price input and output
      // differently ($6 vs $10 per 1M); Sparks metering ignores the split.
      let stepInput = 0;
      let stepOutput = 0;
      ...
      for await (const chunk of stream) {
        ...
        if (state.stopRequested) break;
        if (chunk.usage) {
          stepTokens = chunk.usage.total_tokens;
          stepInput = chunk.usage.prompt_tokens ?? 0;
          stepOutput = chunk.usage.completion_tokens ?? 0;
        }
      ...
      if (stepTokens === 0) {
        ...
        stepInput = Math.ceil(inChars / 4);
        stepOutput = Math.ceil(assistantText.length / 4);
        stepTokens = stepInput + stepOutput;
      }
      // Charge this step's tokens; halt if the pool is now spent (monthly OR the
      // hidden daily cap — isOver() covers both).
      const snap = deps.meter.record(stepTokens, { input: stepInput, output: stepOutput });
      if (deps.meter.isOver()) {
        send({
          type: "paywall",
          id: promptId,
    ...
    if (timedOut) {
      ...
      send({ type: "error", id: promptId, message: redactError(err) });
      send({ type: "agent_done", id: promptId, reason: "completed" });
    }
  } finally {
    clearIdle();
  ...
  switch (name) {
    case "write_to_file": {
      ...
      try {
        ...
        // e.g. the per-file size cap or a path-escape guard — hand the reason back
        // to the model as a tool result so it can adapt, instead of aborting the run.
        return `Could not write ${filePath}: ${(err as Error).message}`;
      }
