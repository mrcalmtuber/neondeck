...
/**
 * M11: hard ceiling on a single file write. Neither the editor-save path nor the
 * agent's writes were bounded, so one arbitrarily large payload could fill the
 * host disk (this is the only disk-fill vector open to the Free tier, which can
 * write files but can't run_command). 20 MB is far above any real source file
 * while still stopping a runaway write. (A full per-workspace quota is a larger
 * follow-up; this closes the single-giant-file case.)
 */
const MAX_WRITE_BYTES = 20 * 1024 * 1024;

function assertWriteSize(content: string): void {
  if (Buffer.byteLength(content, "utf8") > MAX_WRITE_BYTES) {
    throw new Error(`File is too large to save (limit ${MAX_WRITE_BYTES / (1024 * 1024)} MB).`);
  }
}
