...
function client(config: DaemonConfig): OpenAI | null {
  ...
  // Bound the request like the agent loop (agent.ts) so a hung provider connection
  // can't wedge explain/fix on "contacting model" while still billing tokens (M2).
  return new OpenAI({
...
export async function explain(
  ...
  if (!c) {
    send({ type: "error", id, message: "Kryct Agent isn't configured on the daemon." });
  ...
  try {
    ...
    send({ type: "error", id, message: redactError(err) });
  }
...
export async function fix(
  ...
  const c = client(config);
  if (!c) {
    return send({ type: "error", id, message: "Kryct Agent isn't configured on the daemon." });
  }
  try {
