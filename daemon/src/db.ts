...
/** A point-in-time consistent copy of the project's storage.db, for the Firestore
 *  snapshot layer. `VACUUM INTO` takes a transactional read snapshot, so the copy
 *  is coherent even while the user's app is writing. With no open handle we open
 *  a throwaway one (which also rolls back any hot journal from a crashed run).
 *  Falls back to a raw byte copy, then null — never throws. */
export function consistentDbCopy(workspaceDir: string, open?: DatabaseSync | null): Buffer | null {
  const abs = path.join(path.resolve(workspaceDir), DB_FILENAME);
  if (!open && !fs.existsSync(abs)) return null;
  // tmpdir target: never pre-exists (VACUUM INTO requires that) and lives outside
  // the workspace, so the file watcher never sees the copy being made.
  const tmp = path.join(os.tmpdir(), `neondeck-db-${randomUUID()}.db`);
  const vacuumInto = (db: DatabaseSync): Buffer => {
  ...
  try {
    if (open) return vacuumInto(open);
    const throwaway = new DatabaseSync(abs);
    try {
      return vacuumInto(throwaway);
    } finally {
      throwaway.close();
    }
  } catch (err) {
    ...
    console.warn(`[db] consistent copy failed (${(err as Error).message}); using raw read`);
    try {
