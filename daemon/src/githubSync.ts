  ...
  if (check.status === 404) {
    const create = await gh(token, "https://api.github.com/user/repos", {
      ...
      body: JSON.stringify({
        ...
        private: true,
        auto_init: true,
        description: "Kryct projects (synced automatically).",
      }),
    });
...
/** Plain (token-free) repo URL — auth rides in an HTTP header via authEnv(). */
function repoUrl(login: string): string {
  return `https://github.com/${login}/${REPO}.git`;
}
/**
 * SECURITY (H6): supply the GitHub token to git as an HTTP `extraHeader` through
 * environment variables (GIT_CONFIG_*), NOT baked into the remote URL. Embedding
 * it in the URL argv leaked it to `ps` / process listings and into logged git
 * error output. The header is `Authorization: Basic base64("x-access-token:TOKEN")`,
 * GitHub's accepted HTTPS git auth. (Needs git ≥ 2.31 for GIT_CONFIG_COUNT.)
 */
...
async function ensureClone(
  ...
  if (!fs.existsSync(path.join(dir, ".git"))) {
    ...
    const clone = await git(
      path.dirname(dir),
      ["clone", repoUrl(login), path.basename(dir)],
      undefined,
      authEnv(token),
    );
    if (clone.code !== 0) throw new Error(`git clone failed: ${clone.out.slice(0, 200)}`);
    // The remote is already token-free (auth rides in a header); keep it explicit.
    await git(dir, ["remote", "set-url", "origin", repoUrl(login)]);
    await git(dir, ["config", "user.email", "neondeck@users.noreply.github.com"]);
    await git(dir, ["config", "user.name", "Kryct"]);
  } else {
    await git(dir, ["pull", "--no-rebase", repoUrl(login), "main"], undefined, authEnv(token));
  }
  return { dir, login };
}
...
export async function pushProject(
  ...
  try {
    ...
    const push = await git(dir, ["push", repoUrl(login), "HEAD:main"], undefined, authEnv(token));
