 ...
 *
 * When a Firebase project id is configured (default: "neondeck-8cbe0", see
 * config.ts), the daemon verifies the browser's Firebase ID token (a JWT)
 * server-side. The
...
export interface AuthedUser {
  userId: string;
  email: string | null;
  /** Whether Firebase considers the email verified. NEVER trust the email for a
   *  privilege decision (e.g. admin) unless this is true — email/password signup
   *  lets anyone claim any address until they prove ownership. */
...
async function verifyFirebaseIdToken(
  ...
  return { uid: claims.sub, email: claims.email ?? null, emailVerified: claims.email_verified === true };
}
...
export async function authenticate(
  ...
  opts: { allowLoopbackDev?: boolean } = {},
): Promise<AuthedUser> {
  // Firebase not configured at all → local dev mode (token ignored). The dev user
  // is local-only and trusted; it has no email so it never satisfies the admin gate.
  if (!authConfigured(config)) {
    return { userId: DEV_USER_ID, email: null, emailVerified: true, mode: "dev" };
  }
  // A supplied token is always verified → real user.
  if (token) {
    const { uid, email, emailVerified } = await verifyFirebaseIdToken(token, config.firebaseProjectId);
    return { userId: uid, email, emailVerified, mode: "firebase" };
  }
  // No token: only allow the dev user when this connection earned loopback
  // dev-trust (gated in server.ts). Otherwise sign-in is required.
  if (opts.allowLoopbackDev) {
    return { userId: DEV_USER_ID, email: null, emailVerified: true, mode: "dev" };
  }
  throw new Error("Authentication required: no Firebase ID token supplied.");
}
 ...
 *  Only needs the id + mode, so callers can pass a partial identity. */
export function userStorageKey(user: Pick<AuthedUser, "userId" | "mode">): string {
