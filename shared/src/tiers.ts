...
/**
 * Developer-program API pricing (display + cost estimates only — the BILLING
 * truth is the two metered Stripe prices in the dashboard, env
 * STRIPE_PRICE_API_INPUT / STRIPE_PRICE_API_OUTPUT). Dollars per 1M tokens.
 */
export const API_PRICE_INPUT_PER_M = 6;
export const API_PRICE_OUTPUT_PER_M = 10;
/** One line quoted everywhere API pricing appears (Dev section, warning modal, email). */
export const API_PRICE_LABEL = `$${API_PRICE_INPUT_PER_M} per 1M input tokens · $${API_PRICE_OUTPUT_PER_M} per 1M output tokens`;
/** Estimated cost in USD for a run's token usage at the published API rates. */
export function apiCostUsd(inputTokens: number, outputTokens: number): number {
  const usd =
    (inputTokens / 1_000_000) * API_PRICE_INPUT_PER_M +
    (outputTokens / 1_000_000) * API_PRICE_OUTPUT_PER_M;
  return Math.round(usd * 10_000) / 10_000; // 4dp — meter granularity, not invoice truth
}

/**
 * Yearly billing — one switcher everywhere plans are shown. Paying yearly takes
 * 16% off the monthly rate (Pro $10 → $8.40/mo billed yearly; Max $20 → $16.80).
 * The BILLING truth is the yearly Stripe prices in the dashboard
 * (env STRIPE_PRICE_PRO_YEARLY / STRIPE_PRICE_MAX_YEARLY); these helpers are the
 * display math and must match those dashboard amounts.
 */
export type BillingInterval = "month" | "year";
export const YEARLY_DISCOUNT = 0.16;
/** Effective per-month price when billed yearly, e.g. 10 -> 8.4. */
export function yearlyPerMonthUsd(monthlyUsd: number): number {
  return Math.round(monthlyUsd * (1 - YEARLY_DISCOUNT) * 100) / 100;
}
/** Total charged for a year up front, e.g. 10 -> 100.8. */
export function yearlyTotalUsd(monthlyUsd: number): number {
  return Math.round(monthlyUsd * 12 * (1 - YEARLY_DISCOUNT) * 100) / 100;
}
/** "$8.40" / "$10" — trims trailing zeros only for whole-dollar amounts. */
...
/** May this tier publish/share apps publicly with no time limit? Paid only —
 *  the Free tier instead gets a 30-day window (see FREE_PUBLISH_TRIAL_MS). */
export function canPublish(tier: Tier): boolean {
  return getTier(tier).canPublish;
}

/**
 * Free-tier publish window: deploys/share links work for this long after the
 * user's FIRST publish; afterwards an upgrade is required to bring the app
 * back online. Paid tiers never expire.
 */
export const FREE_PUBLISH_TRIAL_DAYS = 30;
export const FREE_PUBLISH_TRIAL_MS = FREE_PUBLISH_TRIAL_DAYS * 86_400_000;
...
export function tierForPriceId(
  ...
  for (const t of TIER_LIST) {
    if (!t.stripePriceEnv) continue;
    if (resolveEnv(t.stripePriceEnv) === priceId) return t.id;
    // Yearly variant of the same plan (e.g. STRIPE_PRICE_PRO_YEARLY).
    if (resolveEnv(`${t.stripePriceEnv}_YEARLY`) === priceId) return t.id;
  }
  return null;
}
