...
export async function explain(
  ...
  if (!c) {
    send({ type: "error", id, message: "No DEEPSEEK_API_KEY set in the daemon environment." });
...
export async function fix(
  ...
  const c = client(config);
  if (!c) {
    return send({ type: "error", id, message: "No DEEPSEEK_API_KEY set in the daemon environment." });
  }
  try {
