...
import type { Tier } from "./shared/protocol.js";
...
import { billingEnabled, createCheckoutSession, handleWebhook } from "./billing.js";
  ...
  return async function handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        const user = await authenticate(config, bearer(req) ?? (typeof body.token === "string" ? body.token : undefined));
      ...
      if (path === "/api/me" && req.method === "GET") {
        const user = await authenticate(config, bearer(req));
