...
function slugify(idea: string): string {
  const base = idea
    ...
    .replace(/^-+|-+$/g, "")
    .slice(0, 32);
  return base || `neondeck-${Date.now().toString(36)}`;
}
...
Neon Agent to scaffold your stack.</p>
...
`,
    "main.js": `// ${title}
// Your blank NeonDeck workspace — start building here.
document.getElementById("ping").addEventListener("click", (e) => {
  e.target.textContent = "🚀 building...";
});
console.log("NeonDeck workspace ready");
`,
  };
}
...
/** Seed one of the NeonDeck templates on the daemon and open it in the IDE. */
