...
export function AgentPanel() {
  ...
  const [apiWarnOpen, setApiWarnOpen] = useState(false);
  ...
  // Developer program: an accepted dev with a card can flip the agent onto
  // metered API billing (no Sparks limits — every token is charged).
  const apiBillingAvailable = !!dev && dev.status === "accepted" && dev.cardOnFile;
  const billedToApi = apiBillingAvailable && !!dev && dev.billInIde;

  async function setApiBilling(on: boolean) {
    setApiBusy(true);
    try {
      setDev(await daemon.devSetBillInIde(on));
    } catch (err) {
      console.warn("[dev] toggle failed:", err);
    } finally {
      setApiBusy(false);
    }
  }
  ...
  // instead of revealing how little is left. Dismissible until reload. Irrelevant
  // while the agent bills to the API key (there is no Sparks limit to near).
  const nearLimit = !billedToApi && !!usage && isNearLimit(usage.tokensUsed, usage.tokensLimit);
  ...
  return (
    <div className="agent">
      ...
      {!billedToApi && tokenMultiplierForEffort(tier, effectiveEffort) > 1 && (
      ...
      {billedToApi && (
        <div className="api-billing-pill">
          ⚡ No limits — metered API billing ($ per use, charged to your card)
        </div>
      )}
      ...
      <div className="agent-input">
        ...
        <div className="agent-actions">
          ...
          {/* Developer program: flip the agent onto metered API billing (no
              Sparks limits, every use charged). Turning ON requires the
              acknowledgement modal; OFF is immediate. */}
          {apiBillingAvailable && (
            <button
              className={`api-billing-chip${billedToApi ? " on" : ""}`}
              disabled={agentRunning || apiBusy}
              title="Bill agent usage to your API key instead of Sparks"
              onClick={() => {
                if (billedToApi) void setApiBilling(false);
                else setApiWarnOpen(true);
              }}
            >
              API billing: {billedToApi ? "On" : "Off"}
            </button>
          )}
      ...
      {apiWarnOpen && (
        <ApiBillingWarning
          ...
          busy={apiBusy}
          onConfirm={() => {
            setApiWarnOpen(false);
            void setApiBilling(true);
          }}
          onCancel={() => setApiWarnOpen(false)}
        />
      )}
    </div>
  );
