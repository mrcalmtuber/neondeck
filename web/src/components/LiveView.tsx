      ...
      <div className="iframe-canvas">
        {previewUrl ? (
          <iframe
            ...
            title="App Preview"
            src={previewUrl}
            // SECURITY: the preview runs UNTRUSTED user/AI-generated code. We only
            // grant `allow-same-origin` when previews are served from a SEPARATE
            // origin (IDE_PREVIEW_ORIGIN) — there it's a different origin than the
            // IDE, so the app can use its own storage but still can't read the IDE's
            // localStorage / auth tokens. Without a separate origin the preview is
            // same-origin as the IDE, so we DROP allow-same-origin (opaque origin)
            // to stop it reaching window.parent's storage — the token-theft path.
            sandbox={
              daemon.info.previewOrigin
