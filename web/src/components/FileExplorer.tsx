...
/** Coarse pointer (touch) — double-click is awkward there, so files open on a
 *  single tap and the pane switches to the editor. */
const TOUCH =
...
function TreeNode({ node, depth }: { node: FileNode; depth: number }) {
  ...
  async function openInEditor() {
    ...
    // The editor lives in the agent pane — on a phone, switch so it's visible.
    if (TOUCH) useStore.getState().setMobilePane("agent");
  }

  // Single click selects (and toggles folders). On desktop, files only OPEN in
  // the editor on double-click — keeping single-click as a lightweight "select";
  // on touch devices a single tap opens directly.
